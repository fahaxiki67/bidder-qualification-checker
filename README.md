...
**状态：开发中（WIP，v0.7.0）。** 可靠性闭环（P0.5：状态合并/批次隔离/主体一致性/行业门控/terms 控制/SSRF 重定向）已完成； 架构骨架（P1）、本地 Web UI（P2）、
全国数据源 adapter 骨架（P3）、集团禁入 adapter（P4）、地区插件机制（P5：四川+广东）、证据系统（P6：SHA-256 留痕/回链/人工复核流/名单导入）已完成，
可靠性闭环（P0/P0.5）已完成；
