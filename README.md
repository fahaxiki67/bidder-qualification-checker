...
**状态：v0.21.0（资格审查与离线多投标文件预警已接入；真实官网联调 P3R 仍待人工配合——作业清单见 docs/P3R_CHECKLIST.md，双平台验收操作单见 docs/UAT.md）。** 可靠性闭环（P0.5：状态合并/批次隔离/主体一致性/行业门控/terms 控制/SSRF 重定向）已完成； 架构骨架（P1）、本地 Web UI（P2）、
...
`.txt`、`.md`、`.csv`、`.json`、`.xlsx`；`.xls`、`.docx` 和 PDF 当前不作为输入解析。
Web `/review-bids` 读取的是运行 Web 服务的服务端所在机器上的本地路径；页面无账号鉴权，
默认只监听回环地址。若显式使用 `bqc serve --allow-lan`，仅应在可信网络临时使用。
...
F-05 报价近似等差/等比、F-06 相对控制价下浮率一致、E-01 电子元数据相同、S-01 文件内容一致、
S-02 文本高相似、S-03 文件结构相似、S-04 可比清单项目构成一致、P-01 本地关联线索、
P-02 人员字段重合、P-03 亲属关系线索。E-01 仅比较作者、机器码、MAC、IP、硬盘序列号、
...
- [《中华人民共和国招标投标法》](https://www.samr.gov.cn/zw/zfxxgk/fdzdgknr/bgt/art/2023/art_1f79dd79321441a0831f3aed697b4535.html)
  ...
  文件混装、保证金同账户，以及单位负责人/控股或管理关系等法定边界；这些条款需要完整事实证据和适用程序。
