...
## 快捷方式：侦察工具（把第 1~2 步压到 1 分钟）

    pip install playwright pyyaml && playwright install chromium   # 一次性
    python scripts/recon_portal.py <source_id> --daytime            # 白天执行

自动打开官方入口→填测试名→点查询→截图 2 张并捕获页面发出的接口请求候选
（candidates.json）。**看截图与候选清单后由你人工确认**，确认的 URL 才回填。
（夜间纪律：真实门户请在白天运行本工具；离线自测已用本机假门户验证。）
