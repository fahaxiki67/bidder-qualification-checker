...
v0.23.0-rc.1 ~ rc.4 预发布候选的收口正式版。相对上一正式版 v0.22.0 的累计变化：

### Added

- **离线审查支持扫描 PDF**：无文本层页面以 Poppler+Tesseract 本地 OCR 识别
  （单文件 50 OCR 页/120 秒预算，超出显式标记部分解析），OCR 原文不公开，
  Windows/macOS 发布包内置引擎与语言包。
- **PDF 文档信息字典进入 E-01 元数据线索**：提取 Producer/Creator/Author
  （生成设备/软件、作者），同一生成设备跨投标人一致时触发 E-01 元数据相同提示；
  元数据缺失或异常不阻断解析，仅作人工复核线索。
- 报价标签前置货币金额句式支持：「（¥ 金额）的投标总报价」中金额位于标签之前，
  按最靠近标签的货币金额取值；head 金额吸收「万元/亿元」后缀单位。

### Fixed

- 根目录无前缀文件不再按文件名拆成多家投标人（幽灵投标人假信号）。
- 纵排/转置汇总表报价可提取（标签行+下行金额/转置数值行按位置配对，歧义宁漏勿错）。
- 报价提取不再把工期数字当作总报价：「工期 N 日历天」「工期要求：…N 天内完工」
  等工期语境（±16 字符窗口）中的唯一金额一律放弃。
- OCR 断号金额（如「¥ 797 965 59.18」）不再被截断成小值当报价。
- 迁移 SQL 去动态拼接；关联线索 CSV 重复表头/JSON 非对象行写入 parse_warnings；
  审查根目录子目录误报越界修复；表单校验失败返回可读 HTML 错误页；
  ERROR/TIMEOUT/BLOCKED 警戒态独立配色；窄屏单列布局。
- GBK 编码文本不再被无 BOM utf-16 分支静默解成乱码。
- 口径文档补齐 F-07 接近带逐规则行；预警信号表与已启用规则编号一一对应的回归锁死。

### Verification boundary

- 三平台测试（Windows/macOS/Ubuntu × Python 3.12）、安装包构建和发布制品由 tag
  触发的 Release 工作流复核；全量 356 项测试通过（另有 2 项平台相关跳过）。
- 全国官方平台真实自动查询仍需人工联调（P3R），所有离线审查与关联结果只输出
  人工复核线索，不自动作出串通投标、违法、资格不合格或投标无效认定。
- 真实商业投标资料（F 盘绰斯甲项目三家分包投标）只读隔离试验贯穿 rc.4~正式版
  修复验证；真实公司名与金额不进仓库。
...
- **根目录无前缀文件不再按文件名拆成多家投标人**：此前同一投标人的「报价.txt」「施工组织.md」
  放在审查根目录时会被当作两家投标人互相比较，凭空产生「报价 ↔ 施工组织」式假报价接近信号；
  现按文档口径归入根目录名对应的单一投标人，「投标人__文件名.ext」显式前缀行为不变。
- **纵排/转置汇总表报价可提取**：标签独占一行、金额在下一非空行的单列汇总，以及
  「标签行 + 数值行」转置汇总（CSV/XLSX/TXT/MD 通用），此前一律识别不出报价导致漏报；
  现按位置一一配对，控制价仍只记 `kind=control` 不作主报价。数值行之后紧跟数字行
  （明细数字列）、下一行带税率/百分比语义或多个数字时一律放弃配对，宁可漏报；
  配对证据同时保留标签行与金额行原文及行号，可对照原表复核。
- **关联线索文件格式歧义留痕**：CSV 表头重复列名（仍按最后一个取值参与配对）与
  JSON 非对象行（按行数忽略）此前静默处理，现写入 `parse_warnings` 提示人工核对原始表。
- **报价取值抗干扰加固**：真实标书「（¥ 金额）的投标总报价，工期 1124 日历天」句式此前会把
  工期数字顶替为报价，现优先识别标签前紧跟货币符号的金额（吸收「万元/亿元」后缀，与标签后
  金额口径一致），标签后唯一非工期金额次之；金额前后紧邻保证金/单价/合价/费率/税率/税额等
...
- **口径文档补齐 F-07 逐规则行**：F-07（`QUOTE_NEAR_BAND`，0.5%~2% 接近带）此前已启用且
  README 有提及，但 `docs/OFFLINE_REVIEW.md` 预警信号表缺该行，复审人员看不到观察内容与
  人工复核材料；现补齐，并新增文档一致性回归——信号表必须与已启用规则编号一一对应（防漏防废）。

### Verification boundary

- 本版为预发布候选；三平台测试、安装包构建和发布制品由 tag 触发的 Release 工作流复核。
- 全国官方平台真实自动查询仍需人工联调，所有离线审查与关联结果仍只输出人工复核线索，
  不自动作出串通投标、违法、资格不合格或投标无效认定。

## [0.23.0-rc.3] - 2026-09-18

### Fixed

- 离线审查现在允许审查根目录下的子目录，而不再把合法的投标资料子目录误报为越界。
- 表单校验失败回到可读的 HTML 错误页，保留用户输入并给出修复指引，不再裸输出 JSON。
- `ERROR`、`TIMEOUT`、`BLOCKED` 警戒态使用独立于正常无数据状态的颜色语义；窄屏页面改为单列布局。
...
- PDF 文本读取扩至 2000 页，流式计算文件摘要；OCR/页数/字符预算、失败页及未处理页明确记录，部分解析文件不参与全文相似度比较。
- 支持 PDF 同页独立标签与紧邻金额；文本 `123.456` 等歧义数字不再静默放大为千分位。
- OCR 原文不进入公开结果，金额仍须对照原页人工复核；Windows/Mac 发布构建内置识别引擎与中英文语言包，增加清空外部工具路径的扫描件验收。

## [0.22.0] - 2026-09-08

### Added
- **PDF 投标文件支持**：`.pdf` 经济标/商务标直接进入离线审查——提取文本层走既有报价与
  相似度规则（上限 500 页，超出如实标注截断）；扫描件（无文本层）显式提示"未纳入比较"，
  不假装已解析。pypdf 转为运行时依赖。
- **F-07 报价接近二档预警**：两家主报价相对差异在 0.5%~2% 时提示（中）；≤0.5% 仍为
  F-01（高）。真实陪标报价差异常落在 0.5%~2% 区间，原单档阈值漏报。

### Fixed
- **「注：…」说明行不再产出报价/控制价**：真实资料中"注：本表适用于建设项目招标控制价
  或投标报价的汇总"曾被当作控制价取值（2.0/3.0 假值），价格指数说明行曾被当作"总报价
  22.0"触发 F-01 假阳性；说明行现统一跳过，不构成报价证据。
- **修复离线审查高影响边界**：XLSX 原生数值不再先转文本，避免三位小数被误读为千分位；
  公开结果统一遮盖报价、清单、关联线索原文中的银行/保证金账号；P-04 对冲突人员/企业字段保守跳过；
  P-04 与主流程共用配对表别名，避免重复触发。
- 新增上述反例回归测试；本版本仍只输出人工复核线索，不自动作出串通、违法或投标无效认定。
- 以上修复的发现途径：龙泉洺悦御府项目一标段（3 家）与二标段（2 家）真实投标资料
  本机只读试验；原始文件零改动，转换与结果均留在 gitignored `local/`。

## [0.21.1] - 2026-09-08

### Added
- **P-04 人员长表识别**：relations 文件支持企查查/公示导出风格的「人员+企业(+职务)」
  长表，同一人任职记录命中至少两家投标人名称即提示（高），无需预先整理成配对表；
  配对表行仍归 P-01/P-03。人员任职重合仅为关联线索，须以工商登记、社保等有权资料核实。
...
- 新增 `app/offline_review_patterns.py` 增量规则模块（已接入 `review_directory`）：
  ...
  交并比高度一致、E-02 银行/保证金账户字段相同、P-02 人员类元数据跨投标人重合、P-03 关联线索中的亲属关系描述、
  S-05 定长分块指纹识别大段共用文本（抓整文比对漏掉的部分抄袭，块内容不进证据）；
  全部为人工复核线索，不自动认定串通、违法或投标无效。

### Fixed
- 外币和负报价不再被静默当作人民币报价或无报价；短/空文本不触发文件完全一致预警；
  JSON 同时存在单价与合价时优先使用合价；同一投标人存在多个冲突控制价时保守跳过 F-06。
- 多字段报价行不再跨列猜测金额；关联 JSON 复用深度/节点和总输入字节预算；CSV/XLSX
  读取受行/单元格边界保护；目录扫描错误保留为可追溯的扫描问题。
- 文本报价行包含多个数值时不再猜测；去空白摘要覆盖全文；XLSX 公式不计算并显式提示缓存值风险；
  输出路径阻断硬链接别名覆盖；Web 同源校验包含协议/端口，`/review-bids` 路径默认限制在
  `BQC_REVIEW_ROOT`（未设置时为服务当前工作目录）内；E-02 账户字段不进入公开结果，避免报告泄露完整账号。
